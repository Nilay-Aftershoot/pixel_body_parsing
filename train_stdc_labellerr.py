...
def parse_args():
    ...
    p.add_argument('--model', type=str, help="The model to train")
...
def train():
    ...
    if args.model == "stdc":
        net = STDC(num_class=n_classes).cuda().train()
    ...
    # LossP = SoftmaxFocalLoss(gamma=0.3) # OhemCELoss(score_thres, n_min, ignore_idx)
    # Loss2 = SoftmaxFocalLoss(gamma=0.3) # OhemCELoss(score_thres, n_min, ignore_idx)
    # Loss3 = SoftmaxFocalLoss(gamma=0.3) # OhemCELoss(score_thres, n_min, ignore_idx)
